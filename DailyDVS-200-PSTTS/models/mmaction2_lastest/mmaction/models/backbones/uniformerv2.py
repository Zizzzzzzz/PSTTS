...
class ResidualAttentionBlock(BaseModule):
    ...
    def forward(self, x: torch.Tensor, T: int = 8, index_token: torch.Tensor = None) -> torch.Tensor:
        # x: 1+HW, NT, C
        ...
        # MHSA
        # tcm = tcm.squeeze(2)
        # index_token = torch.topk(tcm, k=0.3, dim=0, largest=True, sorted=False)[1]
        # index_token = index_token.unsqueeze(1)
        # index_token2 = index_token.repeat(1, 1, x.shape[-1])
        XX = x.clone()
        ...
        x = XX.to(x.dtype).scatter_(0, index_token, xx.to(x.dtype))
        return x
...
class Transformer(BaseModule):
    ...
    def forward(self, x: torch.Tensor, tcm: torch.Tensor) -> torch.Tensor:
        T_down = self.T
        L, NT, C = x.shape
        N = NT // T_down
        H = W = int((L - 1)**0.5)
        ...
        # class_pad = torch.ones(1, NT).to(tcm.dtype) * torch.max(tcm)
        tcm = torch.cat([class_pad, tcm], dim=0)
        index_token = torch.topk(tcm, k=math.ceil(0.488*hw+1), dim=0, largest=True, sorted=False)[1]
        index_token = index_token.unsqueeze(2)
        index_token = index_token.repeat(1, 1, x.shape[-1])
        # print("index_token:", index_token.shape)
        # print("x:", x.shape)
        assert index_token.shape[1] == x.shape[1]
        assert index_token.shape[2] == x.shape[2]
        

        j = -1
        for i, resblock in enumerate(self.resblocks):
            x = resblock(x, T_down, index_token)
            if i in self.return_list:
                ...
                tmp_x = tmp_x.view(index_token.shape[0], N, T_down, C)
