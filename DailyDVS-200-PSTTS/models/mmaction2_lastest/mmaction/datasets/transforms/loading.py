...
class RawFrameDecode(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        for i, frame_idx in enumerate(results['frame_inds']):
            ...
            frame_idx += offset
            if modality == 'RGB':
                x_filepath = osp.join(directory, filename_tmpl.format(frame_idx))
                y_filepath = x_filepath.replace('frames', 'TCMframesnn')
                # y_filepath = x_filepath.replace('MINIHARDVS_EVENT_files-frames0p25', 'MINIHARDVS_EVENT_files-frames0p25-TCM')
                if not os.path.exists(y_filepath):
                    print(y_filepath)
                    y_frame = np.zeros((self.height, self.width), dtype=np.uint8)
                else:
                    y_img_bytes = self.file_client.get(y_filepath)
                    y_frame = mmcv.imfrombytes(y_img_bytes, flag='grayscale')
                y_frame = np.expand_dims(y_frame, axis=2)
                x_img_bytes = self.file_client.get(x_filepath)
                x_frame = mmcv.imfrombytes(x_img_bytes, channel_order='rgb')
                imgs.append(np.concatenate((x_frame, y_frame),axis=-1))
            elif modality == 'Flow':
