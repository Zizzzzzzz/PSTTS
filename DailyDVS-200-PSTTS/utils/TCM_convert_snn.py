...
def construct(x: th.Tensor, y: th.Tensor, pol: th.Tensor, time: th.Tensor) -> th.Tensor:
    ...
    # return merge_channel_and_bins(representation)
        ...
        if os.path.exists(input_filename):
            ...
            with AedatFile(input_filename) as f:
                ...
                for i in range(int(aps_frames_NUM)):     
                    # if os.path.exists(output_file +'/{:08d}.png'.format(i)):
                    #     continue
                    ...
                    TCM = construct(torch.from_numpy(rec_events['x']), torch.from_numpy(rec_events['y']), torch.from_numpy(rec_events['polarity']), torch.from_numpy(rec_events['timestamp']))
                    TCM = TCM.to('cuda')
                    output = []
                    for j in range(TCM.shape[0]):
                        ...
                        sample = inp_img.squeeze(0).squeeze(0).detach().cpu().numpy()
